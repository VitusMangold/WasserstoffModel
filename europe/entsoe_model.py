...
def costs(capacity_power, capacity_h2, share_renewable):
    ...
    missing_power_other = -(other_net.mask(other_net > 0.0, 0.0).to_numpy() + np.where(de_net < 0.0, absolute_inflow, 0.0))
...
result = minimize(lambda x: costs(x[0], x[1], 0.9), x0, bounds=bounds)
...
z_grid = np.array([costs(i, j, 0.9) for i in x_grid for j in y_grid]).reshape(len(x_grid), len(y_grid))
