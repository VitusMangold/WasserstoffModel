function find_optimum(model; scenario, n_chunks=60)
    ...
    elseif scenario == :no_cap # don't use capacities, optimize shares
        trunc = input -> input[n_leaves + 1:end]
        expand = input -> vcat(fill(0.0, n_leaves), input)
    elseif scenario == :no_cap_same # the same share for every country and don't use capacities
    ...
    # Solution
    results = solve(prob, NLopt.LN_NELDERMEAD(), maxtime=3600) # 150s
