...
# df_gen_pq = pd.read_json(mypath + "gen_pq_2035.json")
# df_load_pq = pd.read_json(mypath + "load_pq_2035.json")

desired_scn = "Status Quo"
# desired_scn = "NEP 2035"
def filter_version_scn(df):
    return df.loc[(df["scn_name"] == desired_scn) & (df["version"] == "v0.4.6")]
...
load_ids_pq = set(df_load_pq['load_id'])
print("Missing gen time series:")
print(df_gen[~df_gen["generator_id"].isin(gen_ids_pq)])
# df_gen = df_gen[df_gen["generator_id"].isin(gen_ids_pq)]
print("Missing load time series:")
print(df_load[~df_load["load_id"].isin(load_ids_pq)])
df_load = df_load[df_load["load_id"].isin(load_ids_pq)]
